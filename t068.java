...
class CShape
{
	...
	{
		int k,times;  CShape temp;  k=objArr.length-1;  while(k!=0)
		{
			times=0;
			for(int i=0;i<=k-1;i++)
			{
				...
				{
					temp=objArr[i];	objArr[i]=objArr[i+1];  objArr[i+1]=temp;	times=i;
				}
			}
			k=times;
		}
...
{
	protected double length,width;  public CRect(double i,double j)
	{
		length=i;  width=j;
	}
...
{
	protected double radius;  protected final double pi=3.14;  public CCircle(double i)
	{
		radius=i;
	}
	@Override public void computeArea()
